...
const server = setupServer(
  ...
  http.get(`${API}/device-service/deviceregistration/devices`, ({ request }) => {
    record(request);
  ...
  http.get(`${API}/device-service/deviceservice/device-info/settings/111`, ({ request }) => {
    record(request);
    return HttpResponse.json({ alarms: [{ alarmId: 1 }] });
  }),
  http.get(`${API}/device-service/deviceservice/device-info/settings/222`, ({ request }) => {
    record(request);
    return HttpResponse.json({ alarms: null });
  }),

  http.get(`${API}/web-gateway/device-info/primary-training-device`, ({ request }) => {
    record(request);
    return HttpResponse.json({ deviceId: 111 });
  }),

  http.get(`${API}/web-gateway/solar/111/2026-09-15/2026-09-15`, ({ request }) => {
    record(request);
    return HttpResponse.json({ deviceSolarInput: [{ calendarDate: "2026-09-15", solarUtilization: 12 }] });
  }),
  http.get(`${API}/web-gateway/solar/111/2026-09-10/2026-09-15`, ({ request }) => {
    record(request);
    return HttpResponse.json({ deviceSolarInput: [{ calendarDate: "2026-09-10" }] });
  }),
  http.get(`${API}/web-gateway/solar/111/2026-09-16/2026-09-16`, ({ request }) => {
    record(request);
    return HttpResponse.json({ somethingElse: true });
  }),
  http.get(`${API}/web-gateway/solar/111/2026-09-17/2026-09-17`, ({ request }) => {
    record(request);
...
describe("getDevices", () => {
  it("hits /device-service/deviceregistration/devices and returns an array", async () => {
    ...
    expect(seen[0]!.url).toBe(`${API}/device-service/deviceregistration/devices`);
...
describe("getDeviceSettings", () => {
  it("hits the device-info/settings/{device_id} path with a re-stringified id", async () => {
    ...
    expect(seen[0]!.url).toBe(`${API}/device-service/deviceservice/device-info/settings/111`);
  ...
  it("throws before any request for a non-positive device id", async () => {
    await expect(makeGarmin().getDeviceSettings(0)).rejects.toThrow(GarminError);
    await expect(makeGarmin().getDeviceSettings(-5)).rejects.toThrow(GarminError);
    expect(seen).toHaveLength(0);
  });

  it("throws before any request for a non-numeric device id", async () => {
    await expect(makeGarmin().getDeviceSettings("abc123")).rejects.toThrow(GarminError);
    expect(seen).toHaveLength(0);
  });
});
...
describe("getPrimaryTrainingDevice", () => {
  it("hits /web-gateway/device-info/primary-training-device", async () => {
    ...
    expect(seen[0]!.url).toBe(`${API}/web-gateway/device-info/primary-training-device`);
...
describe("getDeviceSolarData", () => {
  it("defaults enddate to startdate and sends singleDayView=true when enddate is omitted", async () => {
    ...
    expect(result).toEqual([{ calendarDate: "2026-09-15", solarUtilization: 12 }]);
  });

  it("sends singleDayView=false when enddate is explicitly supplied", async () => {
    const result = await makeGarmin().getDeviceSolarData(111, "2026-09-10", "2026-09-15");
    expect(seen[0]!.url).toBe(
