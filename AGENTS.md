...
| `getActivityTypes` | `(): Promise<ActivityTypesResponse \| null>` — passes through unchecked | yes |
| `deleteActivity` | `(activityId: number \| string): Promise<unknown>` — UNCERTAIN upstream null handling (see gotchas); irreversible, never live-tested | **no** — implemented, not live-verified |
| `setActivityName` | `(activityId: number \| string, activityName: string): Promise<unknown>` — UNCERTAIN upstream null handling | **no** — implemented, not live-verified |
| `setActivityType` | `(activityId: number \| string, typeId: number, typeKey: string, parentTypeId: number): Promise<unknown>` — UNCERTAIN upstream null handling | **no** — implemented, not live-verified |
| `setActivityDescription` | `(activityId: number \| string, description: string): Promise<unknown>` — UNCERTAIN upstream null handling | **no** — implemented, not live-verified |
| `createManualActivityFromJson` | `(payload: Record<string, unknown>): Promise<unknown>` — sends `payload` to Garmin verbatim, no shape validation; UNCERTAIN upstream null handling | **no** — implemented, not live-verified |
| `createManualActivity` | `(startDatetime: string, timeZone: string, typeKey: string, distanceKm: number, durationMin: number, activityName: string): Promise<unknown>` — **converts units**: `distanceKm * 1000` → meters, `durationMin * 60` → seconds, before building the request body; `startDatetime` is NOT routed through `formatDate` (it's a full local timestamp, not a bare calendar date) | **no** — implemented, not live-verified |
| `importActivity` | `(file: Blob, filename: string): Promise<ImportActivityResult>` — multipart upload to `/upload-service/upload/{ext}` (extension from `filename`, must be `fit`/`gpx`/`tcx`) with the load-bearing `NK`/`origin`/custom `User-Agent` headers that make Garmin treat it as an import rather than a device sync; a 409 is re-raised as `GarminConnectionError` ("Activity already exists (duplicate): ...") | **no** — implemented, not live-verified |
...
| `upload` | `(file: Blob, filename: string, path?: string, options?: Pick<ApiOptions,"timeoutMs">): Promise<unknown>` | `path` defaults to `/upload-service/upload`. Multipart, field name `file`; `Content-Type` is left unset so `fetch` sets the boundary. Default timeout 60s. **Not live-verified.** |
